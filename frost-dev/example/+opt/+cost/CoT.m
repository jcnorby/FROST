...
for i=1:numel(domains)
    ...
    if ~isnan(getJointIndices(domain, 'tail_joint'))
        resistiveLossTail = Ra/(kt*R).^2*u(9)*u(9);
    else
        resistiveLossTail = 0;
    end
    ...
    if any(strcmp(fieldnames(domain.Params), 'r'))
        power_fun = SymFunction(['powerOverTime_' sys.Gamma.Nodes.Name{i}],CoTi,{u,dx,d,R});
    else
        power_fun = SymFunction(['powerOverTime_' sys.Gamma.Nodes.Name{i}],CoTi,{u,dx,d});
    end
