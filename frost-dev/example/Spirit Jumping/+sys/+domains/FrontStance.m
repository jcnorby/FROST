 ...
 %
 % Contact: Back toes
function domain = FrontStance(model, load_path)
...
%     end
    ...
    if ~isempty(fric_coef)
        ...
        f0 = domain.Inputs.ConstraintWrench.fFoot0;
        % get the friction cone constraint
        %         [friction_cone, fc_label, auxdata] = getFrictionCone(left_foot, f, fric_coef);
        mu = SymVariable('mu');
        gamma = SymVariable('gamma');
        % x, y, z, roll, yaw
        constr = [f0(3) - 0; % fz >= 0
            f0(1) + (mu/sqrt(2))*f0(3);  % -mu/sqrt(2) * fz < fx
            -f0(1) + (mu/sqrt(2))*f0(3); % fx < mu/sqrt(2) * fz
            f0(2) + (mu/sqrt(2))*f0(3);  % -mu/sqrt(2) * fz < fu
            -f0(2) + (mu/sqrt(2))*f0(3)]; % fy < mu/sqrt(2) * fz
    ...
    if ~isempty(fric_coef)
        ...
        f2 = domain.Inputs.ConstraintWrench.fFoot2;
        % get the friction cone constraint
        %         [friction_cone, fc_label, auxdata] = getFrictionCone(left_foot, f, fric_coef);
        mu = SymVariable('mu');
        gamma = SymVariable('gamma');
        % x, y, z, roll, yaw
        constr = [f2(3) - 0; % fz >= 0
            f2(1) + (mu/sqrt(2))*f2(3);  % -mu/sqrt(2) * fz < fx
            -f2(1) + (mu/sqrt(2))*f2(3); % fx < mu/sqrt(2) * fz
            f2(2) + (mu/sqrt(2))*f2(3);  % -mu/sqrt(2) * fz < fu
            -f2(2) + (mu/sqrt(2))*f2(3)]; % fy < mu/sqrt(2) * fz
    ...
    nf_nsf0 = UnilateralConstraint(domain,f0(3),'Foot0NormalForce','fFoot0');
    domain = addEvent(domain, nf_nsf0);
    
    nf_nsf2 = UnilateralConstraint(domain,f2(3),'Foot2NormalForce','fFoot2');
    domain = addEvent(domain, nf_nsf2);
